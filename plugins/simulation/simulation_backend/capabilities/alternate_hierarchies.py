...
class AlternateHierarchyProvider:
    ...
    async def _bop_plan(self, payload, context):
        ...
        if not outcome.ok:
            ...
            # The insert dialog intentionally exposes working/active BOPs as well as
            # published ones.  The official projection endpoint must reject drafts,
            # so resolve the current revision and use the governed preview endpoint
            # for an explicitly selected active version instead of failing blindly.
            if code in {"version_not_published", "bop_version_not_published"}:
                version_gid = str(payload.get("version_gid") or "")
                version_outcome = await client.invoke(DomainInvocation(
                    capability_id="craft.bop.version.get", major_version=1,
                    payload={"version_gid": version_gid}),
                    identity, CorrelationRef(request_id=request_id, trace_id=request_id))
                if not version_outcome.ok:
                    version_code = version_outcome.error.code if version_outcome.error else code
                    raise CapabilityBusinessError(
                        "bop_version_resolution_failed", "bop_version_resolution_failed",
                        details={"source_code": version_code}, retryable=True,
                    )
                version_data = dict(version_outcome.data or {})
                revision = version_data.get("revision")
                if isinstance(revision, bool) or not isinstance(revision, int) or revision < 1:
                    raise CapabilityBusinessError("bop_revision_unavailable", "bop_revision_unavailable")
                outcome = await client.invoke(DomainInvocation(
                    capability_id="craft.bop.execution_structure.preview", major_version=1,
                    payload={"version_gid": version_gid, "expected_revision": revision},
                    expected_resource_version=str(revision)),
                    identity, CorrelationRef(request_id=request_id, trace_id=request_id))
            if not outcome.ok:
                final_code = outcome.error.code if outcome.error else "bop_execution_structure_failed"
                raise CapabilityBusinessError(final_code, final_code, retryable=True)
...
def specs(provider: AlternateHierarchyProvider | None = None):
    ...
    source_ref={"type":"object","properties":{name:{"type":"string"} for name in (
        "document_gid","content_sha256","source_identity_hash","artifact_id","artifact_version",
        "connector_device_id","node_key","occurrence_id","occurrence_gid","occurrence_key",
        "stable_identity","projection_identity","resource_gid","resource_type","resource_code",
        "resource_name","resource_version","source_projection_hash")},"additionalProperties":False}
    placement={"type":"object","required":["placement_gid","hierarchy_gid","workspace_gid","target_node_gid",
        "parent_placement_gid","source_kind","source_ref","transform","display_name","sort_order","row_version"],
        "properties":{"placement_gid":gid,"hierarchy_gid":gid,"workspace_gid":gid,"target_node_gid":nullable_gid,
            "parent_placement_gid":nullable_gid,"source_kind":{"type":"string"},"source_ref":source_ref,
            "transform":{"type":"array","minItems":16,"maxItems":16,"items":{"type":"number"}},
            "display_name":{"type":"string"},"sort_order":{"type":"integer"},"row_version":{"type":"integer","minimum":1}},
    ...
    return (
        ...
        (CapabilitySpec(id="simulation.environment.placement.create",version=1,description="Place a model-document occurrence or resource reference under one hierarchy node.",risk="write",confirmation="none",idempotent=True,input_schema={"type":"object","required":["hierarchy_gid","target_node_gid","source_kind","source_ref","transform","expected_row_version","idempotency_key"],"properties":{"hierarchy_gid":gid,"target_node_gid":gid,"parent_placement_gid":{"anyOf":[gid,{"type":"null"}]},"source_kind":{"type":"string","enum":["vm_occurrence","plmxml","jt","resource"]},"source_ref":source_ref,"transform":{"type":"array","minItems":16,"maxItems":16,"items":{"type":"number"}},"display_name":{"type":"string","maxLength":255},"expected_row_version":{"type":"integer","minimum":1},"idempotency_key":key},"additionalProperties":False},output_schema=placement_output,**common),selected.create_placement),
