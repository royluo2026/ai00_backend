...
def stream_chat(body: dict, token: str) -> Iterator[str]:
    ...
    try:
        ...
        with httpx.Client(timeout=timeout, trust_env=False) as client:
            ...
            payload = {"text": body.get("message", ""), "context": context}
            with client.stream(
                "POST", f"{_base_url()}/v1/sessions/{quote(session_gid, safe='')}/messages/stream",
    ...
    try:
        with httpx.Client(timeout=180.0, trust_env=False) as client:
            ...
            response = client.post(
                f"{_base_url()}/v1/sessions/{quote(session_gid, safe='')}/messages",
                headers=_headers(token), json={"text": body.get("message", ""), "context": body.get("context") or {}},
            )
            response.raise_for_status()
            return {"answer": response.json()["data"]["text"], "session_id": session_gid, "tool_calls": [], "model": "pi"}
    except Exception as exc:
...
def list_tools(token: str) -> dict:
    ...
    read = [{"name": spec["id"], "description": spec.get("description", ""), "category": "read", "need_confirm": False, "params": list(spec.get("input_schema", {}).get("properties", {}))} for spec in specs if spec.get("risk") == "read" and spec.get("confirmation") == "none" and spec.get("execution") == "cloud"]
