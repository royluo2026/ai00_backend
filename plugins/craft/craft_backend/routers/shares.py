...
def get_list_shares(list_gid: str, current_user: dict = Depends(get_current_user)):
    with get_conn() as conn:
        ...
        with conn.cursor() as cur:
            cur.execute(
                "SELECT * FROM workmanship_work_list_shares WHERE list_gid = %s ORDER BY created_at",
                (list_gid,),
            )
            shares = [dict(r) for r in cur.fetchall()]
    users = get_user_summaries(row.get("shared_to") for row in shares)
    for row in shares:
        user = users.get(str(row.get("shared_to")), {})
        row["shared_to_name"] = user.get("name")
        row["shared_to_avatar"] = user.get("avatar_url")
