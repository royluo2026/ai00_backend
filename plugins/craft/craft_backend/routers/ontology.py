...
def _pg_entity_table_to_mysql(pg_table: str) -> str:
    ...
    return craft_entity_table_name(pg_table)
...
def get_class_schema(node_type: str, _u=Depends(get_current_user)):
    ...
    from ..rule_engine.graph import get_ancestor_gids
...
def validate_entry(entry_gid: str, _u=Depends(get_current_user)):
    ...
    from ..rule_engine.reasoner import consistency_check
...
def get_agent_schema(_u=Depends(get_current_user)):
    ...
    from ..rule_engine.reasoner import build_agent_schema
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            if include_inherited:
                from ..rule_engine.reasoner import get_ancestor_gids
