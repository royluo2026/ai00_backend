...
from backend.platform_sdk.auth import get_current_user, get_current_user_optional, derive_org_role
...
def _check_list_access(conn, list_gid: str, user_gid: str, user_team_id: str | None = None) -> str:
    ...
    with conn.cursor() as cur:
        ...
        if read_scope == "global":
            return "read"
        if read_scope == "team" and user_team_id and user_team_id == lst.get("team_id"):
            return "read"
    return "none"
...
def resolve_share_link(
    ...
    with get_conn() as conn:
        ...
        if link["target_type"] == "list":
            current_perm = _check_list_access(conn, link["target_gid"], user_gid, current_user.get("team_id"))
...
def delete_share_link(token: str, current_user: dict = Depends(get_current_user)):
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            org_role = current_user.get("org_role") or derive_org_role(current_user.get("system_role", "external"))
