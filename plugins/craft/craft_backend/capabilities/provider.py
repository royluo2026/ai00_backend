...
_RESOURCE_FIELDS: dict[str, tuple[tuple[str, str], ...]] = {
    ...
    "craft.bop.repository.get": (("craft-bop-repository", "repository_gid"),),
    "craft.bop.repository.archive": (("craft-bop-repository", "repository_gid"),),
    "craft.bop.repository.restore": (("craft-bop-repository", "repository_gid"),),
    "craft.bop.repository.delete": (("craft-bop-repository", "repository_gid"),),
    "craft.bop.repository_baseline.set": (("craft-bop-repository", "repository_gid"),),
    "craft.bop.space.search": (("craft-bop-repository", "repository_gid"),),
    "craft.bop.space.get": (("craft-bop-space", "space_gid"),),
    "craft.bop.space_version.search": (("craft-bop-space", "space_gid"),),
    "craft.bop.managed_personal_space.delete": (("craft-bop-space", "space_gid"),),
    "craft.bop.change_proposal.get": (("craft-bop-proposal", "proposal_gid"),),
    "craft.bop.change_proposal.submit": (("craft-bop-proposal", "proposal_gid"),),
    "craft.bop.change_proposal.review": (("craft-bop-proposal", "proposal_gid"),),
    "craft.bop.change_proposal.apply": (("craft-bop-proposal", "proposal_gid"),),
    "craft.bop.change_proposal.withdraw": (("craft-bop-proposal", "proposal_gid"),),
    "craft.bop.change_proposal.cancel": (("craft-bop-proposal", "proposal_gid"),),
    "craft.bop.change_proposal.supersede": (("craft-bop-proposal", "proposal_gid"),),
    "craft.bop.managed_personal_space.sync.apply": (("craft-bop-space", "personal_space_gid"),),
    "craft.bop.managed_personal_space.import.preview": (("craft-bop-space", "personal_space_gid"),),
    "craft.bop.managed_personal_space.import.apply": (("craft-bop-space", "personal_space_gid"),),
}
...
_CRAFT_BUSINESS_DEFINITIONS = {
    "craft.bop.picture.upload": (
        "Store one bounded image and return its governed URL for later attachment to a BOP entry or process view.",
        (
            "The accepted payload is an image MIME type whose decoded content does not exceed five megabytes.",
            "A successful result contains one URL conforming to the closed upload-result contract.",
            "Invalid filenames, MIME types or base64 content return a governed error without reporting an uploaded asset.",
        ),
        "This capability stores an image asset but does not itself attach or mutate any BOP business entity.",
    ),
    "craft.bop.entry.change.apply": (
        "Create, update or delete one governed BOP entry together with its entry-owned relationship projection.",
        (
            "The target entry belongs to the requested BOP version and caller-visible scope.",
            "Create and update return the persisted entry projection at the resulting BOP revision.",
            "Delete retires the entry and its active entry links in one Craft transaction without leaving visible dangling relationships.",
        ),
        "BOP entry identity, revision and relationship consistency are enforced by the Craft entry-change Provider transaction.",
    ),
