...
def read_bop_alt_hierarchy(payload: dict[str, Any], _context: CapabilityContext) -> CapabilityOutput:
    ...
    if raw_types is None:
        node_types: list[str] = []
    elif isinstance(raw_types, list) and len(raw_types) <= 20:
        node_types = [str(value).strip() for value in raw_types if str(value).strip()]
    else:
        raise ValueError("node_types must be an array with at most 20 items")
    with get_craft_conn() as conn:
        with conn.cursor() as cur:
            clauses = ["e.version_gid=%s", "e.is_deleted=FALSE"]
            ...
            cur.execute(
                ...
                "WHERE " + " AND ".join(clauses) + " ORDER BY e.sort_order LIMIT 1000",
                params,
            )
