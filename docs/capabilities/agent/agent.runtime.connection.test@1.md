# agent.runtime.connection.test@1

Runs a bounded fixed model connectivity probe and returns a normalized diagnostic.

## 使用判断

- 适用：Runs a bounded fixed model connectivity probe and returns a normalized diagnostic.
...
```json
{
  "capability_id": "agent.runtime.connection.test",
