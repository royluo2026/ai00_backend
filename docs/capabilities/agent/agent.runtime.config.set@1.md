# agent.runtime.config.set@1

Replaces explicitly enabled deployment runtime settings without returning the secret.

## 使用判断

- 适用：Replaces explicitly enabled deployment runtime settings without returning the secret.
...
- 授权策略：`agent.v2:system.tech_config`
- 自动化等级：`A1`
- 数据分类：`confidential`
- Delegation：`scoped`
- 认证新鲜度：0 秒
...
```json
{
  "capability_id": "agent.runtime.config.set",
