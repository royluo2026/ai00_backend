# agent.session.list@1

Lists the latest 50 conversations owned by the authenticated user.

## 使用判断

- 适用：Lists the latest 50 conversations owned by the authenticated user.
...
```json
{
  "capability_id": "agent.session.list",
