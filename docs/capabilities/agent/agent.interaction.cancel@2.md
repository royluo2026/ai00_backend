...
Cancels the current interaction only in a session owned by the authenticated user.

## 使用判断

- 适用：Cancels the current interaction only in a session owned by the authenticated user.
