# knowledge.proposal.review@1

审核知识提案；通过后将 Markdown 写入 OIS 并生成正式知识条目。

## 使用判断

- 适用：审核知识提案；通过后将 Markdown 写入 OIS 并生成正式知识条目。
...
资源选择器：
- `knowledge-proposal` ← `proposal_gid`（必填）
...
```json
{
  "capability_id": "knowledge.proposal.review",
