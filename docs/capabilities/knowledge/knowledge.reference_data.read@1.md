# knowledge.reference_data.read@1

Read or publish immutable versioned reference datasets.

## 使用判断

- 适用：Other domains need low-frequency maintained tabular data for reproducible calculations.
...
- 授权策略：`knowledge.v2:knowledge.reference_data.read`
- 自动化等级：`A2`
- 数据分类：`confidential`
- Delegation：`scoped`
- 认证新鲜度：0 秒
...
```json
{
  "capability_id": "knowledge.reference_data.read",
