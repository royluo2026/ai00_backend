# craft.bop.version.snapshot.change.apply@1

Fork an active BOP version into a baseline or promoted immutable snapshot.

## 使用判断

- 适用：A governed Craft consumer freezes or promotes the current active version into a snapshot.
- 不适用：The request toggles frozen links, publishes an existing baseline, or edits draft content.
- 生命周期：`stable`
- 所属领域：`craft`
...
```json
{
  "capability_id": "craft.bop.version.snapshot.change.apply",
