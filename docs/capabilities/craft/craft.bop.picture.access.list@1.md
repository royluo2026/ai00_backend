# craft.bop.picture.access.list@1

Authorize the authenticated reader to view the bounded historical picture set attached to one visible BOP version.

## 使用判断

- 适用：Authorize the authenticated reader to view the bounded historical picture set attached to one visible BOP version.
- 不适用：Another export, import, resource or remote method is requested.
- 生命周期：`experimental`
- 所属领域：`craft`
...
```json
{
  "capability_id": "craft.bop.picture.access.list",
    ...
    "items": {
      "items": {
        ...
        "properties": {
          ...
          "download_url": {
            "maxLength": 4096,
            "pattern": "^https://[^\\s]{1,4088}$",
            "type": "string"
          },
          "media_type": {
            "pattern": "^image/[a-z0-9.+-]{1,64}$",
            "type": "string"
          },
          "reference_hash": {
            "pattern": "^[a-f0-9]{64}$",
