# craft.rule.definition.update@1

Update one mutable Craft rule definition.

## 使用判断

- 适用：A browser consumer changes bounded fields on one mutable rule definition.
- 不适用：The request changes an immutable rule release or waiver.
- 生命周期：`stable`
- 所属领域：`craft`
...
- 授权策略：`craft.v2:craft.rule.write`
- 自动化等级：`A1`
- 数据分类：`confidential`
- Delegation：`scoped`
- 认证新鲜度：0 秒
...
```json
{
  "capability_id": "craft.rule.definition.update",
