...
Governed craft.bop.active_line.validate active BOP line outcome.

## 使用判断

- 适用：A responsibility editor needs an explicit active BOP line.
- 不适用：The caller wants to mutate BOP content.
- 生命周期：`stable`
- 所属领域：`craft`
...
```json
{
  "capability_id": "craft.bop.active_line.validate",
...
- `invalid_cursor`：invalid cursor（retryable=false）
- `invalid_page_size`：invalid page size（retryable=false）
- `resource_not_found`：resource not found（retryable=false）
- `bop_line_wrong_project`：bop line wrong project（retryable=false）
- `bop_line_inactive`：bop line inactive（retryable=false）
- `graph_limit_exceeded`：graph limit exceeded（retryable=false）
