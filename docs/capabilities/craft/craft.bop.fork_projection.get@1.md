# craft.bop.fork_projection.get@1

Read the immutable BOP snapshot projection used by one completed fork.

## 使用判断

- 适用：Read the immutable BOP snapshot projection used by one completed fork.
...
```json
{
  "capability_id": "craft.bop.fork_projection.get",
