# craft.bop.draft.change.apply@1

Apply one exact typed BOP draft preview atomically.

## 使用判断

- 适用：Apply one exact typed BOP draft preview atomically.
...
资源选择器：
- `craft-bop-preview` ← `preview_gid`（必填）
