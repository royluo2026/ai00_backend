...
Apply one idempotent source-scoped BOP line responsibility projection.

## 使用判断

- 适用：The management center needs this exact Base-owned responsibility outcome.
- 不适用：The resource belongs to Project or Craft.
- 生命周期：`stable`
- 所属领域：`base`
...
- 授权策略：`base.org_management:system.user.manage`
- 自动化等级：`A1`
- 数据分类：`confidential`
- Delegation：`scoped`
- 认证新鲜度：0 秒
...
```json
{
  "capability_id": "base.project_responsibility.projection.apply",
...
- `invalid_input`：The closed Base request is invalid.（retryable=false）
- `permission_denied`：The actor cannot perform this Base operation.（retryable=false）
- `resource_not_found`：The Base resource does not exist or is inactive.（retryable=false）
- `version_conflict`：The aggregate revision changed.（retryable=false）
- `idempotency_conflict`：The idempotency key belongs to another payload.（retryable=false）
- `provider_unavailable`：A required owning-domain provider is unavailable.（retryable=true）
