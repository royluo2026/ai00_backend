# base.export_template.update@1

Updates only the named fields of one stored template after proving owner or administrator authority.

## 使用判断

- 适用：Updates only the named fields of one stored template after proving owner or administrator authority.
...
- 授权策略：`base.desktop:authenticated_owner`
- 自动化等级：`A1`
- 数据分类：`confidential`
- Delegation：`none`
- 认证新鲜度：0 秒
...
```json
{
  "capability_id": "base.export_template.update",
