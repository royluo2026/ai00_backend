# base.feishu.event.search@1

Finds bounded matching Feishu calendar events available to the authenticated user.

## 使用判断

- 适用：Finds bounded matching Feishu calendar events available to the authenticated user.
...
```json
{
  "capability_id": "base.feishu.event.search",
