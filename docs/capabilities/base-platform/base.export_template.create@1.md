# base.export_template.create@1

Creates one bounded export template with the authenticated user as its immutable owner.

## 使用判断

- 适用：Creates one bounded export template with the authenticated user as its immutable owner.
...
```json
{
  "capability_id": "base.export_template.create",
