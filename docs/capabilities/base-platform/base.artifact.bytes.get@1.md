# base.artifact.bytes.get@1

Reads one owned immutable artifact for preview with size and hash verification.

## 使用判断

- 适用：Reads one owned immutable artifact for preview with size and hash verification.
...
```json
{
  "capability_id": "base.artifact.bytes.get",
