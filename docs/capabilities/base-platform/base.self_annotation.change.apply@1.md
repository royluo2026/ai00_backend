# base.self_annotation.change.apply@1

Execute exact Base outcome base.self_annotation.change.apply.
...
- `attachment_not_visible`：base.self_annotation.change.apply can return attachment_not_visible.（retryable=false）
- `idempotency_conflict`：base.self_annotation.change.apply can return idempotency_conflict.（retryable=false）
- `invalid_input`：base.self_annotation.change.apply can return invalid_input.（retryable=false）
- `revision_conflict`：base.self_annotation.change.apply can return revision_conflict.（retryable=false）
