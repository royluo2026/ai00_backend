# base.file_store.ois_config.set@1

Saves the explicit OIS connection settings without returning credentials.

## 使用判断

- 适用：Saves the explicit OIS connection settings without returning credentials.
...
```json
{
  "capability_id": "base.file_store.ois_config.set",
