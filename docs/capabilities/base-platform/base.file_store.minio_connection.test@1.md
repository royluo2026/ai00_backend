# base.file_store.minio_connection.test@1

Checks the configured MinIO connection and returns a bounded diagnostic outcome.

## 使用判断

- 适用：Checks the configured MinIO connection and returns a bounded diagnostic outcome.
...
```json
{
  "capability_id": "base.file_store.minio_connection.test",
