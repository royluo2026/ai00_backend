# base.identity.current.get@1

Returns the active user profile and current grants without credentials.

## 使用判断

- 适用：Returns the active user profile and current grants without credentials.
...
```json
{
  "capability_id": "base.identity.current.get",
