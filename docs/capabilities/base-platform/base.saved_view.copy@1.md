# base.saved_view.copy@1

Execute exact Base outcome base.saved_view.copy.
...
```json
{
  "capability_id": "base.saved_view.copy",
...
- `idempotency_conflict`：base.saved_view.copy can return idempotency_conflict.（retryable=false）
- `invalid_input`：base.saved_view.copy can return invalid_input.（retryable=false）
- `legacy_config_unsupported`：base.saved_view.copy can return legacy_config_unsupported.（retryable=false）
- `resource_not_found`：base.saved_view.copy can return resource_not_found.（retryable=false）
