...
Generate one immutable node-level difference report from two visible VM checkpoints.

## 使用判断

- 适用：Generate one immutable node-level difference report from two visible VM checkpoints.
...
资源选择器：
- `simulation-vm-checkpoint` ← `before_checkpoint_gid`（必填）
- `simulation-vm-checkpoint` ← `after_checkpoint_gid`（必填）
...
```json
{
  "capability_id": "simulation.vm_diff_report.generate",
  "catalog_release": "<catalog_release>",
  "major_version": 1,
  "payload": {
    "after_checkpoint_gid": "1",
    "algorithm_version": "example",
    "before_checkpoint_gid": "1",
...
- `vm_checkpoint_not_found`：One or both source checkpoints are unavailable.（retryable=false）
- `vm_diff_workspace_mismatch`：The checkpoints belong to different workspaces.（retryable=false）
- `vm_diff_report_not_found`：The report is unavailable or unreadable.（retryable=false）
