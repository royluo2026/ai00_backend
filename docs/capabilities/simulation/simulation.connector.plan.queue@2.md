...
Queue one immutable compatible execution plan for the bound AI00 Connector as a Simulation user.

## 使用判断

- 适用：A Simulation workflow has an exact version-pinned local plan.
- 不适用：Connector compatibility or session preflight has not passed.
- 生命周期：`experimental`
- 所属领域：`simulation`
        ...
        "steps": {
          "items": {
            ...
            "properties": {
              ...
              "operation_id": {
                "const": "vismockup.document.snapshot@1",
                "type": "string"
              },
              "payload": {
                "additionalProperties": false,
