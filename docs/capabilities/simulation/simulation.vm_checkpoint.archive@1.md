...
Archive a governed VM checkpoint without deleting its evidence.

## 使用判断

- 适用：Archive a governed VM checkpoint without deleting its evidence.
...
资源选择器：
- `simulation-vm-checkpoint` ← `checkpoint_gid`（必填）
...
```json
{
  "capability_id": "simulation.vm_checkpoint.archive",
