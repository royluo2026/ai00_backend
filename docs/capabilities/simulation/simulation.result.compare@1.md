# simulation.result.compare@1

Compare immutable Simulation result references.

## 使用判断

- 适用：Compare immutable Simulation result references.
...
资源选择器：
- `simulation-run` ← `left_result_ref.run_id`（必填）
- `simulation-run` ← `right_result_ref.run_id`（必填）
