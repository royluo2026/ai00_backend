...
Preview a deterministic first-pass BOP-to-VM binding draft without mutating either domain.

## 使用判断

- 适用：Preview a deterministic first-pass BOP-to-VM binding draft without mutating either domain.
...
```json
{
  "capability_id": "simulation.environment.bop_vm_binding_draft.preview",
