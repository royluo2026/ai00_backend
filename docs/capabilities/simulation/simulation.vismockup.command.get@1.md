...
Read one caller-scoped direct VisMockup command outcome.

## 使用判断

- 适用：The caller needs authoritative progress for a queued direct VisMockup command.
- 不适用：The command belongs to another user or tenant.
- 生命周期：`experimental`
- 所属领域：`simulation`
...
资源选择器：
- `simulation-connector-command` ← `operation_id`（必填）
...
```json
{
  "capability_id": "simulation.vismockup.command.get",
