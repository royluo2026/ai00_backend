...
Bind one VM occurrence to one simulation workspace node.

## 使用判断

- 适用：Bind one VM occurrence to one simulation workspace node.
...
```json
{
  "capability_id": "simulation.environment.binding.create",
