# simulation.connector.pairing.bootstrap.create@1

Create a short-lived one-time Connector bootstrap ticket for the signed-in user.

## 使用判断

- 适用：The Simulation page is starting a local Connector handoff.
- 不适用：A Connector is submitting device proof.
- 生命周期：`experimental`
- 所属领域：`simulation`
...
```json
{
  "capability_id": "simulation.connector.pairing.bootstrap.create",
