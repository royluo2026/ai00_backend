...
Reserve a new App runtime generation for a user-confirmed replacement instance.

## 使用判断

- 适用：The device owner confirms replacement of a stale App instance.
- 不适用：Work is unresolved or device identity does not match the authenticated owner.
- 生命周期：`experimental`
- 所属领域：`simulation`
...
```json
{
  "capability_id": "simulation.connector.runtime.takeover",
