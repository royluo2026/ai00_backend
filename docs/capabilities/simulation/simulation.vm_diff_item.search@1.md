...
Page through atomic caller-visible VM difference items.

## 使用判断

- 适用：Page through atomic caller-visible VM difference items.
...
资源选择器：
- `simulation-vm-diff-report` ← `report_gid`（必填）
...
```json
{
  "capability_id": "simulation.vm_diff_item.search",
