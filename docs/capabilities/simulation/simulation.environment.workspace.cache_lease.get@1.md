# simulation.environment.workspace.cache_lease.get@1

Authorize one caller to reuse one exact cached simulation workspace projection for at most five minutes.

## 使用判断

- 适用：Authorize one caller to reuse one exact cached simulation workspace projection for at most five minutes.
...
```json
{
  "capability_id": "simulation.environment.workspace.cache_lease.get",
...
- `workspace_not_found`：The requested workspace is unavailable or no longer readable.（retryable=false）
- `cache_lease_ttl_invalid`：The requested cache lease lifetime must be between 60 and 300 seconds.（retryable=false）
- `cache_lease_signing_key_unavailable`：The server cannot issue authenticated cache leases until signing material is configured.（retryable=false）
