...
Dispatch exactly one downstream action using its separately issued user confirmation.

## 使用判断

- 适用：Dispatch exactly one downstream action using its separately issued user confirmation.
...
```json
{
  "capability_id": "simulation.capture_run.dispatch",
