# simulation.connector.pairing.bootstrap.get@1

Read the signed-in user's safe bootstrap ticket state.

## 使用判断

- 适用：The Simulation page is showing Connector handoff progress.
- 不适用：Secret bootstrap material is required.
- 生命周期：`experimental`
- 所属领域：`simulation`
...
```json
{
  "capability_id": "simulation.connector.pairing.bootstrap.get",
