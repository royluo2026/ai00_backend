...
Prepare and queue one frozen environment package, opening only its generated top-level PLMXML.

## 使用判断

- 适用：A user replays one exact frozen Simulation environment in the bound AI00 App runtime.
- 不适用：The environment is editable, not frozen, or the runtime is not the App v2 Connector.
- 生命周期：`experimental`
- 所属领域：`simulation`
