...
Prepare one deterministic frozen VisMockup package whose generated top-level PLMXML is the only document opened.

## 使用判断

- 适用：Prepare one deterministic frozen VisMockup package whose generated top-level PLMXML is the only document opened.
...
```json
{
  "capability_id": "simulation.environment.runtime_package.prepare",
