...
Atomically insert the exact previewed BOP process skeleton as one alternate hierarchy after revalidating the Craft source and plan hash.

## 使用判断

- 适用：Atomically insert the exact previewed BOP process skeleton as one alternate hierarchy after revalidating the Craft source and plan hash.
...
```json
{
  "capability_id": "simulation.environment.bop_projection.apply",
