...
Dispatch the prepared snapshot action using its separate user confirmation.

## 使用判断

- 适用：Dispatch the prepared snapshot action using its separate user confirmation.
...
```json
{
  "capability_id": "simulation.document_snapshot.dispatch",
