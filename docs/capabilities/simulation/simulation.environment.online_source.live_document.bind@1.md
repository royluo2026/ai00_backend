...
Bind a freshly launched Teamcenter online source document to its attested VisMockup session.

## 使用判断

- 适用：A user-confirmed Teamcenter launch has opened a new VisMockup document for an owned online-source model.
- 不适用：Either signed launch evidence or current native document identity is unavailable or mismatched.
- 生命周期：`experimental`
- 所属领域：`simulation`
...
```json
{
  "capability_id": "simulation.environment.online_source.live_document.bind",
