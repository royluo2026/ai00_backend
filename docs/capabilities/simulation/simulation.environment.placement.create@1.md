...
Place a model-document occurrence or resource reference under one hierarchy node.

## 使用判断

- 适用：Place a model-document occurrence or resource reference under one hierarchy node.
...
资源选择器：
- `simulation-alternate-hierarchy` ← `hierarchy_gid`（必填）
...
```json
{
  "capability_id": "simulation.environment.placement.create",
