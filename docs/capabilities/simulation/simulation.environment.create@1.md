# simulation.environment.create@1

Create a reproducible environment from four immutable references.

## 使用判断

- 适用：Create a reproducible environment from four immutable references.
...
- `digital-model` ← `model_snapshot_ref.model_id`（必填）
- `digital-model-version` ← `model_snapshot_ref.version_id`（必填）
- `simulation-parameter-set` ← `parameter_set_ref.parameter_set_id`（必填）
