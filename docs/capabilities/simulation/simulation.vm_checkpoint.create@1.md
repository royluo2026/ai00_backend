...
Create a governed personal checkpoint or authorized shared VM baseline from an exact completed snapshot.

## 使用判断

- 适用：Create a governed personal checkpoint or authorized shared VM baseline from an exact completed snapshot.
...
- `simulation-document-snapshot` ← `snapshot_request_id`（必填）
...
```json
{
  "capability_id": "simulation.vm_checkpoint.create",
...
- `workspace_not_found`：The checkpoint workspace is unavailable or unreadable.（retryable=false）
- `document_snapshot_not_found`：The source snapshot request is unavailable to this caller.（retryable=false）
- `document_snapshot_not_completed`：The source snapshot request has not completed.（retryable=true）
- `snapshot_hash_mismatch`：The supplied hash differs from the authoritative completed snapshot.（retryable=false）
- `vm_snapshot_projection_required`：The completed Connector snapshot has not yet been projected into authoritative VM persistence.（retryable=true）
- `vm_checkpoint_shared_forbidden`：The caller cannot create or archive a shared project baseline.（retryable=false）
- `vm_checkpoint_not_found_or_conflict`：The checkpoint is unavailable or its row version changed.（retryable=true）
- `idempotency_conflict`：The idempotency key is bound to different checkpoint content.（retryable=false）
