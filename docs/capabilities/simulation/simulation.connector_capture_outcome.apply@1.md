# simulation.connector_capture_outcome.apply@1

Project one authenticated Connector capture outcome into its exact Simulation capture run.

## 使用判断

- 适用：Project one authenticated Connector capture outcome into its exact Simulation capture run.
...
资源选择器：
- `simulation-capture-run` ← `capture_run_id`（必填）
