...
Read the current user's single Connector binding.

## 使用判断

- 适用：AI00 Web needs to display or select the user's Connector.
- 不适用：A new Connector must be paired.
- 生命周期：`experimental`
- 所属领域：`simulation`
...
```json
{
  "capability_id": "simulation.connector.binding.get",
