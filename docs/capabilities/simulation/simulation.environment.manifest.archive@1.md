# simulation.environment.manifest.archive@1

Archive a Connector environment identity without mutating its manifests.

## 使用判断

- 适用：Archive a Connector environment identity without mutating its manifests.
...
资源选择器：
- `simulation-environment` ← `environment_id`（必填）
...
```json
{
  "capability_id": "simulation.environment.manifest.archive",
