...
Soft-remove one placement subtree from an alternate hierarchy.

## 使用判断

- 适用：Soft-remove one placement subtree from an alternate hierarchy.
...
资源选择器：
- `simulation-placement` ← `placement_gid`（必填）
...
```json
{
  "capability_id": "simulation.environment.placement.remove",
