# simulation.connector_materialization_outcome.apply@1

Project one authenticated Connector materialization outcome into its exact Simulation run.

## 使用判断

- 适用：Project one authenticated Connector materialization outcome into its exact Simulation run.
...
资源选择器：
- `simulation-materialization-run` ← `run_id`（必填）
