# project.approval.order.start@1

Commit approval order start within the authenticated tenant and participant policy.

## 使用判断

- 适用：Commit approval order start within the authenticated tenant and participant policy.
...
```json
{
  "capability_id": "project.approval.order.start",
