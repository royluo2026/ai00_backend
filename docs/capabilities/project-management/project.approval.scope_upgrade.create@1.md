# project.approval.scope_upgrade.create@1

Commit approval scope_upgrade create within the authenticated tenant and participant policy.

## 使用判断

- 适用：Commit approval scope_upgrade create within the authenticated tenant and participant policy.
...
- 授权策略：`project_management.v2:approval.submit`
- 自动化等级：`A1`
- 数据分类：`confidential`
- Delegation：`none`
- 认证新鲜度：0 秒
...
```json
{
  "capability_id": "project.approval.scope_upgrade.create",
