# project.approval.order.search@1

Read approval order search within the authenticated tenant and participant policy.

## 使用判断

- 适用：Read approval order search within the authenticated tenant and participant policy.
...
```json
{
  "capability_id": "project.approval.order.search",
