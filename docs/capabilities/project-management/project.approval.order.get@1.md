# project.approval.order.get@1

Read approval order get within the authenticated tenant and participant policy.

## 使用判断

- 适用：Read approval order get within the authenticated tenant and participant policy.
...
```json
{
  "capability_id": "project.approval.order.get",
