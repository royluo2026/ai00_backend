...
Read bounded project responsibility tree, matrix, or operation status.

## 使用判断

- 适用：The super-admin management center reads manual project responsibilities.
- 不适用：The caller needs ordinary project members.
- 生命周期：`stable`
...
- 授权策略：`project.org_management:project.view`
- 自动化等级：`A2`
- 数据分类：`confidential`
- Delegation：`scoped`
- 认证新鲜度：0 秒
...
```json
{
  ...
  "properties": {
    ...
    "operation": {
      "enum": [
        "responsibility_tree.search",
        "responsibility_matrix.get",
        "operation.get"
      ],
      "type": "string"
    }
...
```json
{
  "capability_id": "project.org_management.read",
  "catalog_release": "<catalog_release>",
  "major_version": 1,
  "payload": {
    "arguments": {},
    "operation": "responsibility_tree.search"
  }
}
```
...
- `invalid_input`：invalid input（retryable=false）
- `permission_denied`：permission denied（retryable=false）
- `resource_not_found`：resource not found（retryable=false）
- `version_conflict`：version conflict（retryable=false）
- `idempotency_conflict`：idempotency conflict（retryable=false）
- `projection_pending`：projection pending（retryable=true）
- `projection_failed`：projection failed（retryable=true）
- `provider_unavailable`：provider unavailable（retryable=true）
