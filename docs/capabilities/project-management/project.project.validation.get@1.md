...
Return a minimal project identity projection for the fixed Base validator.

## 使用判断

- 适用：Base validates a project before replacing its manager set.
- 不适用：A user needs normal project detail.
- 生命周期：`stable`
...
```json
{
  "capability_id": "project.project.validation.get",
