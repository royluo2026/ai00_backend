# project.approval.order.approve@1

Commit approval order approve within the authenticated tenant and participant policy.

## 使用判断

- 适用：Commit approval order approve within the authenticated tenant and participant policy.
...
- 授权策略：`project_management.v2:approval.approve`
- 自动化等级：`A1`
- 数据分类：`confidential`
- Delegation：`none`
- 认证新鲜度：0 秒
...
```json
{
  "capability_id": "project.approval.order.approve",
