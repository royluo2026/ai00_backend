...
- 2026-09-14 regression and live fix: the Teamcenter-launched visible window belongs to the ordinary `VisView_NG.exe` process started at 12:22; a `VisView_NG.exe -Embedding` COM server process started at 15:02 has no top-level windows. The 13:40 change to `WindowsVisMockupCom.TryGetActiveApplication` rejected any non-unique process identity, discarding the COM connection when both processes existed. Test-backend probe plans show `ready=true, document_ready=true` at 13:26 local time before that change, `automation_unavailable` after it, then `ready=true, document_ready=true` at 18:50 after restoring attachment when a process is running and retaining the cached connection while extra processes make the identity ambiguous. The user confirmed the desktop UI reports “已连接”. The focused .NET suite passed 48/48. Neither VisMockup process was stopped. Earlier claims that this Teamcenter window categorically could not be attached were incorrect; `GetActiveObject`/ROT and `OBJID_NATIVEOM` failures alone did not establish that `Activator.CreateInstance` could not recover the working connection.
- G3 still needs an explicit controlled check that the COM document is the intended visible Teamcenter document when multiple models/windows exist; the successful probe proves an active automation document, not a PID-to-document binding.
