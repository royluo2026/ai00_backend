...
The v2 wire contract sets `MAX_JSON_DEPTH=64` for canonicalization, model validation, parsing, hash projections, and signing. Depth counts object and array containers equally, including a root container as depth 1; scalar values do not add depth. Thus the plan/Outcome root object, `steps` array, and step object consume three levels, leaving at most 61 nested containers inside a payload/result. Depth 64 is accepted and depth 65 is rejected explicitly in both Python and .NET, independently of serializer defaults.
