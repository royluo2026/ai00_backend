...
- Outcomes use the pairing-generated device ECDSA P-256 private key. The cloud stores the device public key and verifies each Outcome v2 signature. Results are bound to the leased plan and step. Only the cloud Provider can apply the resulting business transition.
...
Before opening wake-up or leasing, ConnectorHost registers a runtime session using its current device credential and generation. The cloud transactionally replaces `current_runtime_instance_id`, issues a short-lived runtime-session token bound to that instance, and invalidates the prior instance session. Registration, replacement, heartbeat, and lease use compare-and-set semantics on device generation and current instance. A process that loses the race cannot heartbeat, lease, renew, submit an outcome, or reconcile, even when it holds the same generation credential.
...
- Generation-fencing and runtime-session tests that race legacy, stale App, and current App runtimes; atomically replace the current instance; and reject stale-generation or replaced-instance wake-up, heartbeat, lease, renewal, outcome, reconciliation, and rollback calls.
