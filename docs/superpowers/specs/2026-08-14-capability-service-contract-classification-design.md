...
- **Application Use Case（应用用例）**：从应用层观察的完整用户或系统意图；在 AI00 中通常由一个 Service Operation 对外承载。
...
> Catalog 中拥有稳定 ID 和 Major，可以独立授权、调用、版本化、审计和验证的 Service Operation / Application Use Case。
...
Service Contract / Service Interface
    └── defines_operation
            └── Capability（受治理的 Service Operation）
                    ...
                    ├── consumed_by UI / Plugin / Agent Workflow / Automation
