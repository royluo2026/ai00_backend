...
- A local draft Catalog (`.runtime/reverse-capture-catalog-candidate.json`) validates with 771 descriptors and adds exactly `simulation.connector.health.get@2` and `simulation.connector.plan.queue@3` relative to the current working release. It has not been published or substituted for the checked release.
...
4. Offline governance scan is blocked by `pinned_product_descriptor_count_mismatch` (checked release: 546 stable descriptors; scanner pin: 545). Generated Catalog and checked release also differ. The candidate Catalog is only review evidence; a human/server release decision and deterministic scanner update are still required. Do not adjust the pin or publish the release solely to clear this gate.
