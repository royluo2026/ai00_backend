ALTER TABLE `workmanship_sim_connector_pairings`
  ADD COLUMN IF NOT EXISTS `activation_challenge_hash` CHAR(64) NULL AFTER `credential_envelope_hash`;
ALTER TABLE `workmanship_sim_connector_pairings`
  ADD COLUMN IF NOT EXISTS `activation_status` VARCHAR(32) NOT NULL DEFAULT 'not_issued' AFTER `activation_challenge_hash`;
ALTER TABLE `workmanship_sim_connector_pairings`
  ADD COLUMN IF NOT EXISTS `activated_at` DATETIME(6) NULL AFTER `completed_at`;

ALTER TABLE `workmanship_sim_connector_bindings`
  ADD COLUMN IF NOT EXISTS `activated_at` DATETIME(6) NULL AFTER `last_seen_at`;
ALTER TABLE `workmanship_sim_connector_bindings`
  ADD COLUMN IF NOT EXISTS `pending_pairing_id` VARCHAR(128) NULL AFTER `activated_at`;
