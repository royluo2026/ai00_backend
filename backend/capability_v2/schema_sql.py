...
EXPORT_SQL = """-- Read-only structural export for DBeaver. Target: `{database}`.
...
PREFLIGHT_SQL = """-- STOP unless every result below matches the checklist. Target: `{database}`.
...
SELECT DATABASE() = '{database}' AS database_ok,
...
def _database_sql(template: str, database_name: str) -> str:
...
def _order_missing_tables(tables: list[TableSpec]) -> list[TableSpec]:
    """Create referenced missing tables before tables that own foreign keys."""
    ...
    while remaining:
        ready = sorted(
            ...
            if not {
                ...
                for constraint in by_name[name].constraints
                if constraint.kind == "foreign_key"
                and constraint.referenced_table in remaining
            }
        )
        if not ready:
            raise SchemaPlanError(
                "missing_table_dependency_cycle:" + ",".join(sorted(remaining))
            )
...
def _checklist(diff: SchemaDiff | None, export_only: bool, database_name: str) -> str:
    if export_only:
        return f"# Schema export checklist\n\n1. In DBeaver select `{database_name}`.\n2. Run `00-export-schema.sql`.\n3. Export the three result sets using the exact CSV names.\n"
...
def build_execution_package(*, expected: ExpectedSchema, diff: SchemaDiff | None,
    ...
    _quote(expected.database_name)
    if diff and (
        diff.database_name != expected.database_name
        or diff.expected_sha256 != expected.schema_sha256
    ):
        raise SchemaPlanError("expected_schema_identity_mismatch")
    export_sql = _database_sql(EXPORT_SQL, expected.database_name)
    documents: dict[str, bytes] = {
        "00-export-schema.sql": export_sql.encode(),
        "expected-schema.json": _json(expected.to_dict()),
        "execution-checklist.md": _checklist(diff, export_only, expected.database_name).encode(),
    }
    if not export_only:
        ...
        missing_tables: list[TableSpec] = []
        missing_columns, missing_indexes = [], []
        for item in diff.safe:
            if item.kind == "missing_table": missing_tables.append(item.expected_object)
        ...
        quoted_database = _quote(expected.database_name)
        banner = f"-- Expected {expected.schema_sha256}; target {quoted_database}.\nUSE {quoted_database};\n"
        documents.update({
            "01-preflight.sql": _database_sql(PREFLIGHT_SQL, expected.database_name).encode(),
            "10-create-missing-tables.sql": (
                banner + "\n\n".join(_table(table) for table in _order_missing_tables(missing_tables)) + "\n"
            ).encode(),
