...
class PersonalActivityOwnershipTests(unittest.TestCase):
    def test_personal_follow_and_notification_tables_are_base_owned(self):
        registry = json.loads((ROOT / "backend/governance/domain_boundaries.json").read_text(encoding="utf-8"))
        self.assertEqual(registry["table_overrides"]["workmanship_work_follows"], "base")
        self.assertEqual(registry["table_overrides"]["workmanship_work_notifications"], "base")
