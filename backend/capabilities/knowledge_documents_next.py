...
def register_knowledge_document_capabilities(registry) -> None:
    ...
    registry.register(CapabilitySpec(id="knowledge.space.list", version=1, description="List spaces visible to the current user.", permissions=("knowledge.view",), input_schema={"type":"object"}, output_schema={"type":"object"}, tags=("knowledge","space","read")), list_spaces)
    registry.register(CapabilitySpec(id="knowledge.document.create", version=1, description="Create a published Markdown document revision in immutable OIS storage.", risk="write", confirmation="user", idempotent=False, permissions=("knowledge.manage",), input_schema={"type":"object","required":["space_gid","title","slug","markdown"]}, output_schema={"type":"object"}, tags=("knowledge","document","write")), create_document)
    registry.register(CapabilitySpec(id="knowledge.document.revise", version=1, description="Publish a new immutable revision of a document.", risk="write", confirmation="user", idempotent=False, permissions=("knowledge.manage",), input_schema={"type":"object","required":["document_gid","markdown"]}, output_schema={"type":"object"}, tags=("knowledge","document","write")), revise_document)
    registry.register(CapabilitySpec(id="knowledge.document.search", version=1, description="Search authorized published document revisions by title or slug.", permissions=("knowledge.view",), input_schema={"type":"object"}, output_schema={"type":"object"}, tags=("knowledge","document","read")), search_documents)
    registry.register(CapabilitySpec(id="knowledge.document.get", version=1, description="Read one authorized immutable Markdown revision with evidence.", permissions=("knowledge.view",), input_schema={"type":"object","required":["document_gid"]}, output_schema={"type":"object"}, tags=("knowledge","document","read")), get_document)
    registry.register(CapabilitySpec(id="knowledge.document.revisions", version=1, description="List authorized immutable revision metadata for a document.", permissions=("knowledge.view",), input_schema={"type":"object","required":["document_gid"]}, output_schema={"type":"object"}, tags=("knowledge","document","read")), list_document_revisions)
    registry.register(CapabilitySpec(id="knowledge.document.diff", version=1, description="Diff two authorized immutable Markdown revisions.", permissions=("knowledge.view",), input_schema={"type":"object","required":["document_gid","from_revision_gid","to_revision_gid"]}, output_schema={"type":"object"}, tags=("knowledge","document","read")), diff_document_revisions)
    registry.register(CapabilitySpec(id="knowledge.document.rollback", version=1, description="Restore a historical revision by publishing a new immutable revision.", risk="write", confirmation="user", idempotent=False, permissions=("knowledge.manage",), input_schema={"type":"object","required":["document_gid","target_revision_gid"]}, output_schema={"type":"object"}, tags=("knowledge","document","write")), rollback_document)
    registry.register(CapabilitySpec(id="knowledge.document.acl.list", version=1, description="List document ACL entries for document administrators.", permissions=("knowledge.manage",), input_schema={"type":"object","required":["document_gid"]}, output_schema={"type":"object"}, tags=("knowledge","acl","read")), list_document_acl)
    registry.register(CapabilitySpec(id="knowledge.document.acl.revoke", version=1, description="Revoke one document ACL entry without removing creator administration.", risk="write", confirmation="user", idempotent=True, permissions=("knowledge.manage",), input_schema={"type":"object","required":["document_gid","subject_type","subject_gid"]}, output_schema={"type":"object"}, tags=("knowledge","acl","write")), revoke_document_acl)
    registry.register(CapabilitySpec(id="knowledge.document.acl.grant", version=1, description="Grant document access inside the current tenant.", risk="write", confirmation="user", idempotent=True, permissions=("knowledge.manage",), input_schema={"type":"object","required":["document_gid","subject_type","subject_gid","permission"]}, output_schema={"type":"object"}, tags=("knowledge","acl","write")), grant_document_acl)
