"""Base-owned Knowledge projections consumed by official domain workbenches."""
...
def list_knowledge_workbench_items(user_gid: str, list_gids: list[str] | None = None) -> list[dict]:
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT 'knowledge' AS item_type,gid,title,status,NULL AS priority,"
                "scheduled_date,NULL AS due_date,created_at,NULL AS project_name,"
                "maintainer_gid AS owner_user_gid FROM workmanship_know_entries "
