...
def main():
    ...
    capabilities.extend(registry.get(row[0],1).descriptor.model_dump(mode='json') for row in [*ARTIFACT_DEFINITIONS,*EXCHANGE_DEFINITIONS])
    ...
    (ROOT / 'backend/tests/fixtures/desktop_round5_base_outcomes.json').write_text(json.dumps({'fixture_kind': 'Real registered owner handlers and services; deterministic SQL/external I/O ports, no live approval', 'rows': rows}, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
    agent_rows = execute_agent_matrix()
    (ROOT / 'backend/tests/fixtures/desktop_round5_agent_outcomes.json').write_text(json.dumps({'fixture_kind':'Real owner handlers/application/repositories; deterministic SQL/HTTP ports','rows':agent_rows},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    project_rows,_ = execute_project_matrix()
    (ROOT / 'backend/tests/fixtures/desktop_round5_project_outcomes.json').write_text(json.dumps({'fixture_kind':'Real Project handlers/application/repositories; deterministic SQL driver; complete create/start/approve/withdraw/replay/notification','rows':project_rows},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    print(f'{len(capabilities)} unpublished candidates; {len(rows)} actual owner handler outcomes')
    file_rows = execute_file_matrix()
    (ROOT / 'backend/tests/fixtures/desktop_round5_file_outcomes.json').write_text(json.dumps({'fixture_kind':'Real Artifact store, CSV/XLSX handlers and Feishu owner services; deterministic HTTP ports','rows':file_rows},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
