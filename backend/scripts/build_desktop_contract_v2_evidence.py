...
def main():
    ...
    candidate={'status':'unpublished_candidate','published_catalog_unchanged':True,'machine_passed':False,'human_approved':False,'runtime_verified':False,'catalog_generation_errors':errors,'catalog_blocker_count':len(errors),'capabilities':[registry.get(i,2).descriptor.model_dump(mode='json') for i in ids], 'residual_schema_blockers':[
        {'capability':'craft.library.change.apply@2','fields':['record.spec','record.meta','meta'],'reason':'Authoritative request model declares untyped dict; nonempty arbitrary JSON requires a named business schema and remains rejected.'},
    ...
    for d in candidate['capabilities']:pages.extend([f"## {d['id']}@2",'',f"Provider: `{d['owner_domain']}`. User confirmation and existing handler retained.",'','```json',json.dumps(d['input_schema'],ensure_ascii=False,indent=2),'```',''])
