...
SECRET_FIELD_TOKENS = ("secret", "private_key", "password", "token")
...
def validate_release_report(path: Path) -> tuple[str, ...]:
    try:
    ...
    public_key = str(document.get("public_key", "")).strip()
    if not public_key:
        errors.add("release_report_public_key_missing")
    ...
    if _has_secret_field(document):
        errors.add("release_report_secret_present")
...
def _has_secret_field(value: object) -> bool:
    if isinstance(value, dict):
        return any(
            any(token in str(key).lower() for token in SECRET_FIELD_TOKENS)
            or _has_secret_field(child)
...
def build_production_artifact(
    ...
    *,
    ...
    errors = validate_release_report(release_report)
    if errors:
    ...
    for relative in _allowed_backend_files(source_root, allowlist):
        _copy(source_root, artifact_root, relative, copied)
    for relative in _allowed_frontend_files(Path(frontend_root), allowlist):
        _copy(Path(frontend_root), artifact_root, relative, copied)
    _copy(Path(release_report).parent, artifact_root, Path(release_report).name, copied, destination="release-report.json")

    exclusion = check_production_artifact(artifact_root)
...
def _load_allowlist(path: Path) -> dict[str, object]:
    ...
    required = {"backend_top_level_packages", "migrations", "frontend_prefixes", "catalog_files", "provider_modules"}
...
def _allowed_frontend_files(root: Path, allowlist: dict[str, object]) -> tuple[str, ...]:
    ...
    for path in root.rglob("*"):
        if not path.is_file():
            continue
        relative = path.relative_to(root).as_posix()
        if not _copyable(relative) or _governance_path(relative):
            continue
