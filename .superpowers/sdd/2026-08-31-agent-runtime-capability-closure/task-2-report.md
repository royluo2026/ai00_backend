...
The runtime performs the database/resource load and the existing `CanvasExecutor` call in one spawned worker. Request and result envelopes are JSON-only. A bounded one-way pipe is drained while the worker is live, so a valid near-limit result cannot deadlock process shutdown. The child caps the serialized envelope before sending and the parent independently caps receipt. Internal and application timeouts terminate and join the process, close process/pipe handles, and leave no late marker or growing active-worker set across repeated runs. No thread-backed blocking work or second executor was added.
...
- RED: a valid near-limit envelope and an oversized envelope both deadlocked the Queue feeder and were misreported as `runtime_timeout`.
...
- GREEN: production process tests prove JSON result transport, near-limit delivery, oversized-envelope rejection, internal timeout cleanup, application-cancellation cleanup, repeated-run worker stability, and concurrency capping.

## Verification evidence

- Focused canvas query suites: `27 passed`.
- Complete Agent provider/runtime/domain suite: `107 passed`.
