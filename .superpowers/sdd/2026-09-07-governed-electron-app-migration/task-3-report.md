...
## Fix round 1/5 — crash expiry and obsolete queued plans

Implementation commit: `49116c32a` (`fix: close Connector session expiry lifecycle gaps`).

Status: **DONE_WITH_CONCERNS**. The two reported lifecycle defects are corrected; native MySQL and the previously documented baseline issues remain unverified/unresolved. This section supersedes the original recovery eligibility and queued-plan behavior above.

The P1 review reproduced a crash where the executor leased a plan and stopped before reporting. After both lease and normal-session expiry, the row remained `leased`/`executing`, and recovery registration previously rejected it because only already-unknown/manual-review rows were accepted. Recovery registration now performs a device-first fenced expiry transition for its named plan: device, protocol, generation, execution instance, original session hash and elapsed lease must match; the normal session must already be expired. It records `outcome_unknown`, `reconciliation_state=pending` and a plan-specific `lease_expired` audit before issuing the plan-scoped recovery session, within the same transaction. It never infers failed-without-effect from an executor crash. A wrong original binding or still-live lease remains unchanged. Failure in either expiry audit or final recovery-registration audit rolls back both plan transition and recovery-session creation.

The P2 review reproduced two queue failures: replacing a session stranded its signed queued plans, and re-registering the same instance with a new token allowed an old plan/hash retry to be acknowledged. Before either expiry-based replacement or forced takeover installs its new identity, the shared installation path now terminalizes every old-session plan that is still `queued`, has zero attempts and no lease. These rows become `failed_without_effect` and each receives a `plan_failed_without_effect` audit with reason `runtime_session_replaced_before_lease`. Signed plan JSON, generation, execution instance and original session hash are unchanged; no signed outcome is fabricated. This cleanup, new-session installation and final session audit are one transaction. Tests force the last audit to violate its primary key and confirm both queued state and old session are restored.

Plan insertion authenticates the current session token before checking the incoming plan and any idempotency match. Existing plan identity/hash can be acknowledged only when the original device/generation/instance/session hash also matches that current session. An obsolete plan or obsolete idempotency key raises `plan_session_mismatch`, including reuse of the same instance string after token rotation and an already-expired old plan. A new equivalent signed plan using the new session and a fresh idempotency key queues and leases normally. Same-session, same-plan retries remain idempotent. No recovery scope, signed execution identity, protocol rule, migration, governance metadata or service surface changed in this fix round.

RED evidence:

- `python -m pytest backend/tests/test_simulation_connector_runtime_v2_sql.py -q --tb=short -k 'crash or replacement or reusing_instance'`: **8 failed, 8 skipped** before implementation. Failures were the two crash statuses rejected as `plan_reconciliation_invalid`, unchanged queued rows after replacement/takeover, an incorrectly acknowledged old-session retry, and missing transactional audit failure points.
- After the first fixes, that same selection: **8 passed, 8 skipped**.
- A further expired-old-plan retry variant reproduced the wrong error precedence (`plan_lease_invalid` instead of `plan_session_mismatch`): **1 failed, 1 passed, 2 skipped**. Moving plan expiry validation after original-session matching fixed the remaining variant without allowing expired new plans.

GREEN evidence:

- Required four-module focused command from section 5: **67 passed, 44 skipped**.
- The broader passing command from section 5, including existing outcome/projection/control-plane tests, migration-file validation and optional native MySQL projection tests: **108 passed, 46 skipped**.
- Additional cases cover expiry-audit and recovery-session-audit rollback independently, every obsolete queued plan, stale generation/instance/hash, a nonexpired lease, replacement/takeover final-audit rollback, signed-plan preservation, obsolete idempotency-key rejection and delivery of the equivalent newly signed request.
- All skips still require the absent `AI00_SIMULATION_TEST_DB_URL`; no native MySQL execution or deployed-runtime verification is claimed. Previously reported unrelated baseline inventory/checksum/HTTP-route failures were not repaired or relabeled as passing.
- Self-reviewed the complete source diff, device-first lock order, exact identity predicates, no-effect qualification (`attempts=0`, no lease), audit ordering, and rollback behavior. `git diff --check` and staged checks passed. No push, merge, publication or subagent dispatch occurred.
