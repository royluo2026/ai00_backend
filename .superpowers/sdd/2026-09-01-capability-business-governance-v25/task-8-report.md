...
## Fix round 4 — canonical acceptance identity and link rejection

Commit `2a7e573e55b5d8f9b11d47882d192624368fdae9` removes the resolve-before-validation gap. The shared repository-input parser now lexically normalizes safe in-root `.`/`..` and platform separator aliases, walks every canonical component with `lstat`, rejects symbolic links and Windows reparse points (including junctions), and only then resolves the file and enforces repository containment, type, and size. The acceptance manifest path and every referenced source use that same parser in both provenance discovery and direct scanning. Emitted test-node IDs, metadata, source paths, and hashed source bytes therefore bind the declared canonical source rather than a resolved target reached through an alias.

Duplicate acceptance identity is now `(canonical root-relative source path, exact source symbol)`, evaluated after secure path parsing. Exact duplicates and dot, safe-dotdot, and Windows backslash aliases of one source and symbol fail closed; distinct exact symbols in the same source remain valid. The complete production manifest still accepts all 3,353 unique references and discovers only its canonical manifest plus `backend/tests/acceptance/test_mandatory_cases.py`. Staged and unstaged dirt tests cover both the manifest and referenced source.

TDD RED was `6 failed, 3 passed, 3 skipped` for the canonical-alias subset, plus a separately forced Windows directory-junction case that failed because the resolved target was accepted. GREEN is `10 passed, 2 skipped`: this host cannot create final-file or manifest-file symlinks without `SeCreateSymbolicLinkPrivilege` (`WinError 1314`), so those tests skip with that explicit reason; the directory-junction/reparse case runs through a `mklink /J` fallback and passes. The full clean Task 8 suite is `39 passed, 2 skipped in 43.63s`. Clean scanner regression is `42 passed, 2 failed in 5.10s`, retaining only the already-recorded offline Integration binding and stale scalar-error wording assertions.

The read-only baseline is `E:/Projects/ai00_v3/.runtime/task8-capability-business-audit-fix4.json` (911,560 bytes; SHA-256 `f5365dc8b2b242d8adf73ab8a72c01584517013321dc0449e900bfa0ef6368b7`). It was generated from detached clean backend and Web materializations and binds snapshot `991`, backend/source `2a7e573e55b5d8f9b11d47882d192624368fdae9`, and Web `b48b630b8ade831f2f8bd31275d7629aeb5af434`. Counts remain 495 findings, 495 root groups, 495 affected Capabilities in 11 domains, 2 remediation families, 1 relation, 17 public unbound entries, and 495 review items. Layers remain A/B/F/G=495 and C/D/E=0; maturity is L1=495; machine passes and human/runtime remain false. The first invocation against the configured dirty Web checkout failed closed; the successful invocation used the exact clean Web revision and no trust check was bypassed. Targeted compile and `git diff --check 0250dd6d..2a7e573e` both passed.
