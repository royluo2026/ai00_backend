...
document.getElementById('btn-sync-feishu').addEventListener('click', async () => {
  ...
  try {
    const res = await api('/feishu/sync/org/structure', { method: 'POST' });
    if (res?.status === 'syncing') {
      banner.textContent = '组织架构同步已在后台启动，完成后请刷新页面查看';
...
function _renderStructTree() {
  ...
  Object.values(childMap).forEach(arr => arr.sort((a, b) => a.name.localeCompare(b.name, 'zh')));

  // 首次渲染时自动展开第一层
  if (_expandedTeams.size === 0 && childMap['__root__']) {
    childMap['__root__'].forEach(t => _expandedTeams.add(t.gid));
  }
  ...
  function renderNode(team, depth) {
    ...
    html += `<div class="st-node${isSelected ? ' selected' : ''}" data-gid="${team.gid}" style="padding-left:${indent}px">
  ...
  async function _doSearch(q) {
    ...
    results.querySelectorAll('.user-search-result-item').forEach(el => {
      el.addEventListener('mousedown', e => {
        e.preventDefault();
        if (!el.dataset.gid) {
          return _toast('该用户尚未注册，请先同步组织架构', 'err');
        }
        _selectUser(el.dataset.gid, el.dataset.name, el.dataset.avatar);
      });
    });
  }

  function _selectUser(gid, name, avatarUrl) {
    results.classList.add('hidden');
    input.value = '';
...
let _selectedTeamMemberGid = null;
const _teamMemberSearch = _makeUserSearchWidget({
  ...
  onSelect:   gid => { _selectedTeamMemberGid = gid; },
});
...
document.getElementById('btn-confirm-team-member').addEventListener('click', async () => {
  if (!_selectedTeamMemberGid) return _toast('请先搜索并选择用户', 'err');
  const res = await api(`/teams/${_selectedTeamGid}/members`, {
    method: 'POST',
    body: JSON.stringify({ user_gid: _selectedTeamMemberGid }),
  });
  if (res?.success) {
    document.getElementById('modal-add-team-member').classList.add('hidden');
