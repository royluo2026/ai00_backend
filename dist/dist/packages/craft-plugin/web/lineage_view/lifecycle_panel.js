...
class BopLifecyclePanel {
  ...
  _renderSideHistoryPanel() {
    ...
    undoBtn.addEventListener('click', async () => {
      try {
        const resp = await this._cf(`/api/bop/versions/${this._versionGid}/lifecycle/lines/${lineGid}/undo`, { method: 'POST' });
        this._toast('已撤销', 'ok');
        // 用后端返回的操作日志直接渲染，避免额外请求
        if (resp.operation_log) {
          this._lastOperationLog = resp.operation_log;
        }
        this._renderSideHistoryPanel();
        await this._load();
    ...
    redoBtn.addEventListener('click', async () => {
      try {
        const resp = await this._cf(`/api/bop/versions/${this._versionGid}/lifecycle/lines/${lineGid}/redo`, { method: 'POST' });
        this._toast('已重做', 'ok');
        if (resp.operation_log) {
          this._lastOperationLog = resp.operation_log;
        }
