...
class LayoutDetailPanel {
  ...
  constructor({ containerEl, cf, toast, patchEntry, reloadData, getLineageData, onNodeActivate, getVersionInfo, onVersionChange }) {
    ...
    this._colDetail    = this._el.querySelector('#llDpDetail');
    this._detailBody   = this._el.querySelector('#llDpDetailBody');
  ...
  open(gid) {
    ...
    if (!data) return;
    if (this._userClosed && !this._isOpen) {
  ...
  async _computeDerivedProp(gid, p) {
    ...
    // Try meta first (cheap — data already in memory)
    let values = children
      .map(c => {
        const meta = (c.meta && typeof c.meta === 'object') ? c.meta : {};
        return meta[child_property];
      })
      .filter(v => v != null)
      .map(Number)
      .filter(v => !isNaN(v));

    // Fallback: fetch entity-props for each child (entity_table properties)
    if (!values.length) {
      const results = await Promise.all(
        children.map(c =>
          this._cf(`/api/bop/entries/${encodeURIComponent(c.gid)}/entity-props`)
            .then(r => r?.data?.[child_property])
            .catch(() => null)
        )
      );
  ...
  _renderDetailEmpty() {
    this._detailBody.innerHTML = `
      <div class="ll-det-empty-wrap">
        <span style="font-size:20px;opacity:.4">←</span>
        <span>点击左侧关系项查看详情</span>
      </div>`;
    this._detMode = 'empty';
  }
  ...
  async _openViewDetail(item, currentGid) {
    ...
    // 关闭
    this._detailBody.querySelector('#llDetClose')?.addEventListener('click', () => this._renderDetailEmpty());
    this._detailBody.querySelector('#llDetCancel')?.addEventListener('click', () => this._renderDetailEmpty());

    // 来源跳转
    this._detailBody.querySelector('#llDetSrcLink')?.addEventListener('click', e => {
      const srcGid = e.target.dataset.gid;
      if (srcGid) this.open(srcGid);
    });

    // 保存
    this._detailBody.querySelector('#llDetSave')?.addEventListener('click', async () => {
      if (!canEditCurrentLine) {
        this._toast?.('当前线体无编辑权限（只读）', 'warn');
        return;
      }
      const fields = {};
      this._detailBody.querySelectorAll('[data-field]').forEach(el => {
      ...
      try {
        if (linkType !== 'child') {
          ...
          const newTitle = this._detailBody.querySelector('#llDetTitleInp')?.value?.trim();
          if (newTitle) {
    ...
    // 解除关联
    this._detailBody.querySelector('#llDetUnlink')?.addEventListener('click', async () => {
      if (!canEditCurrentLine) {
        this._toast?.('当前线体无编辑权限（只读）', 'warn');
        return;
      }
  ...
  async _openAddDetail(key, parentGid, nodeType, typeLabel) {
    ...
    let isPbomType = key === 'pbom' || ['pbom_part', 'part', 'non_standard_part', 'standard_part', 'support_material'].includes(_nt);
    ...
    // 关闭/取消
    const closeAdd = () => this._renderDetailEmpty();
    this._detailBody.querySelector('#llAddClose')?.addEventListener('click', closeAdd);
    this._detailBody.querySelector('#llAddCancel')?.addEventListener('click', closeAdd);

    // ── PBOM 版本切换 → 重新加载零件 ──
    const verSel = this._detailBody.querySelector('#llPbomVerSel');
    if (verSel) {
      verSel.addEventListener('change', async () => {
        const newGid = verSel.value;
        if (!newGid) return;
        selectedPbomGid = newGid;
        const candsEl = this._detailBody.querySelector('#llAddCands');
    ...
    const searchInp = this._detailBody.querySelector('#llAddSearchInp');
    searchInp?.addEventListener('input', () => {
      const q = searchInp.value.trim().toLowerCase();
      this._detailBody.querySelectorAll('.ll-det-sr-item').forEach(item => {
        const fields = [
          item.dataset.title || '',
    ...
    const _bindCandClicks = () => {
      this._detailBody.querySelectorAll('.ll-det-sr-item').forEach(item => {
        item.addEventListener('click', () => {
          this._detailBody.querySelectorAll('.ll-det-sr-item').forEach(i => i.classList.remove('sel'));
    ...
    // ── 确认添加 ──
    this._detailBody.querySelector('#llAddConfirm')?.addEventListener('click', async () => {
      const sel = this._detailBody.querySelector('.ll-det-sr-item.sel');
