'use strict';
const _resourceStagingUrl = versionGid => `/api/craft/tc-resource-staging?version_gid=${encodeURIComponent(versionGid)}`;
const _resourceCandidateUrl = resourceType => `/api/craft/resource-requirements?resource_type=${encodeURIComponent(resourceType)}`;
...
class StagingPanel {
  ...
  async load() {
    ...
    try {
      const response = await this._cf(_resourceStagingUrl(this._versionGid));
  ...
  async resolveResourceItem(item) {
    try {
      const response = await this._cf(_resourceCandidateUrl(item.resource_type));
      ...
      await this._cf(`/api/craft/tc-resource-staging/${item.gid}/resolve`, {
  ...
  async ignoreResourceItem(item) {
    try {
      await this._cf(`/api/craft/tc-resource-staging/${item.gid}/ignore`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
