...
vm.runInNewContext(`${source}\nmodule.exports = { LayoutDetailPanel, _buildRuntimeRelationGroups, _craftResourceArguments, _candidatePrimary, _candidateSearchText };`, sandbox);
const { LayoutDetailPanel, _buildRuntimeRelationGroups: groupsFor, _craftResourceArguments: argumentsFor, _candidatePrimary, _candidateSearchText } = sandbox.module.exports;
...
for (const [resourceType, linkType] of [
  ...
  assert.strictEqual(argumentsFor(group.linkType)?.resource_type, resourceType);
...
assert.strictEqual(argumentsFor('physical_tool'), null);
...
panel._invokeCapability = (id, payload) => {
  ...
  assert.strictEqual(payload?.page_size, 200);
